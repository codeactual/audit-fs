 ...
 * @return {boolean}
 */
rules.minSize = function(name, bytes) {
  return AuditFs.getFileSize(this.resolve(name)) >= bytes;
};
 ...
 * @return {boolean}
 */
rules.maxSize = function(name, bytes) {
  return AuditFs.getFileSize(this.resolve(name)) <= bytes;
};
 ...
 * @return {boolean}
 */
rules.minCount = function(name, count) {
  return AuditFs.getFileCount(this.resolve(name)) >= count;
};
 ...
 * @return {boolean}
 */
rules.maxCount = function(name, count) {
  return AuditFs.getFileCount(this.resolve(name)) <= count;
};
 ...
 *
 * @param {string} name
 * @param {number} max Max age, in seconds, to be considered new
 * @return {boolean}
 */
rules.created = function(name, max) {
  var stats = fs.lstatSync(this.resolve(name));
  return (Date.now() - stats.ctime.valueOf()) <= max;
};
 ...
 * @return {boolean}
 */
rules.modified = function(name, max) {
  var stats = fs.lstatSync(this.resolve(name));
  return (Date.now() - stats.mtime.valueOf()) <= max;
};
