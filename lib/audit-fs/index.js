...
rules.minSize = function(config) {
  return AuditFs.getFileSize(this.resolve(config.name)) >= config.size;
};
...
rules.minCount = function(config) {
  return AuditFs.getFileCount(this.resolve(config.name)) >= config.count;
};
...
rules.created = function(config) {
  var stats = fs.lstatSync(this.resolve(config.name));
